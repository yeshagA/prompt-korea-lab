    ...
    <section className="section-padding">
      <div className="container-main">
        ...
        <div className="mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {categories.map((c) => (
            <div key={c.title} className="bg-card rounded-lg border p-6 flex flex-col justify-between hover:shadow-sm transition-shadow duration-200">
              <div>
                <span className={`inline-block text-xs font-semibold px-2 py-0.5 rounded ${c.color}`}>{c.level}</span>
                <h3 className="mt-3 text-lg font-semibold text-foreground">{c.title}</h3>
                <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{c.desc}</p>
              </div>
              <Button size="sm" className="mt-5 self-start">학습 시작</Button>
            </div>
          ))}
        </div>
      </div>
